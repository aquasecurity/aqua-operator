...
var _ = BeforeSuite(func() {
	...
	if logStdout == "true" {
		logf.SetLogger(zap.New(zap.WriteTo(GinkgoWriter), zap.WriteTo(os.Stdout), zap.UseDevMode(false), zap.Encoder(encoder)))

	} else {
		logf.SetLogger(zap.New(zap.WriteTo(GinkgoWriter), zap.UseDevMode(false), zap.Encoder(encoder)))
	}
...
func PrepareEnv() {
	...
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateNamespace("local-storage"))).Should(Succeed())
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateServiceAccount())).Should(Succeed())
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateClusterRole())).Should(Succeed())
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateClusterRoleBinding())).Should(Succeed())
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateRole())).Should(Succeed())
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateRoleBinding())).Should(Succeed())
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateHostPathProvisionerDeployment())).Should(Succeed())
	//Expect(k8sClient.Create(context.TODO(), test_utils.CreateStorageClass())).Should(Succeed())
}
